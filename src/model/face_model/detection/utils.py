...
def py_nms(dets, thresh, mode="Union"):
    ...
    while order.size > 0:
        ...
        inds = np.where(ovr <= thresh)[0]  # IOU大于阈值的被淘汰，然后选择未被淘汰的第一个
        order = order[inds + 1]
    return keep
...
def generate_bbox(cls_map, reg, scale, threshold):
    ...
    stride = 2
    cellsize = 12
    # 将置信度高的留下 np.where输出符合条件的序号
    t_index = np.where(cls_map > threshold)
    # print(t_index) cls_map是二维的，用np.where返回的分别是行序号和列序号 这里t_index为行序号
    # 没有人脸
    if t_index[0].size == 0:
        return np.array([])
    # 偏移量
