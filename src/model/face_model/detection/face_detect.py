...
class MTCNN:
    def __init__(self, model_path):
        ...
        # 获取R模型
        self.rnet = torch.jit.load('D:/social/QQ/files/1448931856/FileRecv/sapas_app_back/'
                                   'src/model/face_model/save_model/mtcnn/RNet.pth', map_location='cpu')
    ...
    def predict_pnet(self, infer_data):
        ...
        infer_data = torch.unsqueeze(infer_data, dim=0)  # 插入维度1x
        # 执行预测
        cls_prob, bbox_pred, _ = self.pnet(infer_data)
        # print(cls_prob.shape)
        cls_prob = torch.squeeze(cls_prob)  # 将维度中的1x去掉
        # print(cls_prob.shape)
    ...
    def detect_pnet(self, im, min_face_size, scale_factor, thresh):
        ...
        while min(current_height, current_width) > net_size:
            # 类别和box cls_map是每个box的置信度 预测的box是这张变化后图片的位置
